import { Box, Button, Dialog, DialogTitle, IconButton, Paper, Typography } from "@mui/material";
...
function Register () {
    ...
    console.log(name);
      if(name.length <= 5) {
        setMessage({ open: true, text: 'Insira um nome com mais de 5 caracteres', type: 'Erro' });
        ...
        const dados = await axios.post('https://wallet-back-end-rexp.vercel.app/user/register', {
          nomeDeUsuário: name,
          Email: email,
          Senha: password
        });
        if (typeof dados.data === "object") {
          setMessage({ open: true, text: 'Usuario cadastrado com sucesso', type: 'Sucesso' });
    ...
    return (
      ...
      >
        <Paper 
          ...
          >
          <Typography color="primary" fontWeight="bold" variant="h5">Register</Typography>
          ...
          <Box display="flex" flexDirection="column" sx={{ width: {xs: '70%', md: '60%'} }}>
            ...
            <Box display="flex" flexDirection="row" mt={1}>
              <Typography color="#fff" variant="h8" >Já possui cadrastro?</Typography>
              <CSSLink to="/login">Login</CSSLink>
            </Box>
          </Box>
          </Box>
          <Button onClick={handleClick} variant="contained" >registrar</Button>
        </Paper>
        {
          message.type === 'Erro' && (
            <Dialog
